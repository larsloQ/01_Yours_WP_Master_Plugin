        ...
        > 
            ...
            <div className="master-toggle">
              ...
              <ToggleControl
                ...
                onChange={
                  ()=>{
                  /* add block ID to store */
                  var blockID = wp.data.select( 'core/editor' ).getSelectedBlockClientId();
                  var prevblockID = wp.data.select( 'store-master-toggle' ).getCurrentBlockId();
                  console.log("herer:", blockID)
                  console.log("prevblockID:", prevblockID)
                  if (prevblockID!=="" && blockID !== prevblockID) {
                    wp.data.dispatch("core/editor").updateBlockAttributes(prevblockID, {master_settings_open:false});
                  }
                  wp.data.dispatch( 'store-master-toggle' ).addActiveBlock(blockID);
                  // master_settings_open: !master_settings_open
                  setAttributes({master_settings_open:!master_settings_open}); // by default set this to false
                  }
                }
              />
            </div>
